...
export const MimeTypes = {
  ...
  isText: (mimeType: string): boolean => mimeType.startsWith("text/"),
  ...
  isMultipart: (mimeType: string): boolean => mimeType.startsWith("multipart/"),

  getMainType: (mimeType: string): string => mimeType.split("/")[0] || "",

  getSubType: (mimeType: string): string => mimeType.split("/")[1] || "",
} as const;
...
export const BodyUtils = {
  ...
  flattenParts: (bodyParts: EmailBodyPart[]): EmailBodyPart[] => {
    ...
    const traverse = (parts: readonly EmailBodyPart[]) => {
   ...
   */
  markAsSeen: (
    keywords: { readonly [x: string]: boolean } | undefined,
  ): { readonly [x: string]: boolean } => {
    return {
      ...(keywords || {}),
  ...
  toggleFlag: (
    ...
    const isFlagged = currentKeywords[StandardKeywords.FLAGGED] === true;
    return {
      ...currentKeywords,
      [StandardKeywords.FLAGGED]: !isFlagged,
    };
  },
...
export const AttachmentUtils = {
  ...
  filterByType: (
    ...
    mimeType: string,
  ): EmailAttachment[] => attachments.filter((att) => att.type === mimeType),
  ...
  isSafeForInlineDisplay: (attachment: EmailAttachment): boolean => {
    const safeMimeTypes: string[] = [
      MimeTypes.IMAGE_PNG,
      MimeTypes.IMAGE_JPEG,
      MimeTypes.IMAGE_GIF,
      MimeTypes.TEXT_PLAIN,
    ];
    return safeMimeTypes.includes(attachment.type);
  },
};
...
export const HTMLUtils = {
   ...
   */
  stripTags: (html: string): string => html.replace(/<[^>]*>/g, ""),
...
export const BinaryUtils = {
   ...
   */
  toBase64: (data: Uint8Array): string => btoa(String.fromCharCode(...data)),
 ...
 */
export class EmailProcessingError extends Data.TaggedError(
...
export const EmailProcessing = {
  ...
  ): Effect.Effect<string, EmailProcessingError> =>
    Effect.gen(function* () {
